...
#else // !defined(CONFIG_HTC_SDIO_DEBUG)

#define DATA_DEBUG(x, y...)						\
	do {								\
		if (sdio_al->debug.debug_data_on || (sdio_dbg_flag & DBG_SDIO_DATA))			\
			printk(KERN_INFO "[SDIO] "y);					\
		sdio_al_log(x, y);					\
	} while (0)

#define LPM_DEBUG(x, y...)						\
	do {								\
		if (sdio_al->debug.debug_lpm_on || (sdio_dbg_flag & DBG_SDIO_LPM))			\
			printk(KERN_INFO "[SDIO] "y);					\
		sdio_al_log(x, y);					\
	} while (0)
...
#define sdio_al_logi(x, y...)						\
	do {								\
		if (sdio_dbg_flag & DBG_SDIO_INFO) \
			printk(KERN_INFO "[SDIO] "y);						\
		sdio_al_log(x, y);					\
	} while (0)
...
#define CLOSE_DEBUG(x, y...)						\
	do {								\
		if (sdio_al->debug.debug_close_on || (sdio_dbg_flag & DBG_SDIO_CLOSE))			\
			printk(KERN_INFO "[SDIO] "y);					\
		sdio_al_log(x, y);					\
	} while (0)
...
#define pr_debug(x...) do {				\
		if (sdio_dbg_flag & DBG_SDIO_DEBUG) \
			printk(KERN_INFO "[SDIO] "x);		\
	} while (0)
...
static void sdio_func_irq(struct sdio_func *func)
{
	...
	if (sdio_al_dev->is_ok_to_sleep)
		sdio_al_wake_up(sdio_al_dev, 1, NULL);
	else
...
/* +SSD_RIL: remove for speeding up shutdown*/
...
/* +SSD_RIL: remove for speeding shutdown*/
#if !defined(CONFIG_MACH_MECHA)
...
static int __init sdio_al_init(void)
{
	...
	if (get_kernel_flag() & BIT(16)) /* data log */
		sdio_dbg_flag |= 0x10000;
	if (get_kernel_flag() & BIT(17)) /* lpm log */
		sdio_dbg_flag |= 0x20000;
	if (get_kernel_flag() & BIT(18)) /* close log */
		sdio_dbg_flag |= 0x40000;
	if (get_kernel_flag() & BIT(19)) /* debug log */
		sdio_dbg_flag |= 0x80000;
	if (get_kernel_flag() & BIT(20)) /* info log */
		sdio_dbg_flag |= 0x100000;
        /*-SSD_RIL: add htc debug mechanism */
