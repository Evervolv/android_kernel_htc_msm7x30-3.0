...
#define BR_DEF_USER_PWM		143
#define BR_MIN_USER_PWM		30
#define BR_MAX_USER_PWM		255

#define BR_DEF_PANEL_PWM	135
#define BR_MIN_PANEL_PWM	9
#define BR_MAX_PANEL_PWM	255
...
enum led_brightness glacier_brightness_value = DEFAULT_BRIGHTNESS;

static int glacier_shrink_pwm(int brightness)
{
	int level;

	if (brightness <= BR_DEF_USER_PWM) {
		if (brightness <= BR_MIN_USER_PWM)
			level = BR_MIN_PANEL_PWM;
		else
			level = (BR_DEF_PANEL_PWM - BR_MIN_PANEL_PWM) *
				(brightness - BR_MIN_USER_PWM) /
				(BR_DEF_USER_PWM - BR_MIN_USER_PWM) +
				BR_MIN_PANEL_PWM;
	} else {
		level = (BR_MAX_PANEL_PWM - BR_DEF_PANEL_PWM) *
		(brightness - BR_DEF_USER_PWM) /
		(BR_MAX_USER_PWM - BR_DEF_USER_PWM) +
		BR_DEF_PANEL_PWM;
	}
