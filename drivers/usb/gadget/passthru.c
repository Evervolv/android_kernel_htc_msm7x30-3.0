...
#define TCP_DATA_PORT	26857
#define CMD_DEV2PC		0x80020000
#define CMD_PC2DEV		0x00050000		/* It tells us that the current PC tool is version 2. */
...
void ipt_close(void)
{
	...
	int ii;
		...
		kernel_sock_shutdown(ipt->l_sock, SHUT_RDWR);
		USB_DEBUG("[IPT] kernel_sock_shutdown.l_sock\n");
	/* To avoid race condition that rxthread will still access l_sock during sock_shutdown */
		for (ii = 0; ii < 5; ii++) {
			if (ipt->thread == NULL)
				break;
			msleep(10);
			USB_DEBUG("[IPT] wait thread terminated: %d\n", ii);
		}
		/* We put release() here is because accept() also calls it if accept remote connection failed. */
