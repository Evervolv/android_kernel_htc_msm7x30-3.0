...
void vision_snddev_poweramp_on(int en)
{
	...
	if (en) {
		gpio_set_value(PM8058_GPIO_PM_TO_SYS(VISION_AUD_SPK_ENO), 1);
		mdelay(100);
	} else {
		gpio_set_value(PM8058_GPIO_PM_TO_SYS(VISION_AUD_SPK_ENO), 0);
	}
}
...
#if 0
	if (en)
		gpio_request(PM8058_GPIO_PM_TO_SYS(VISION_AUD_CODEC_EN), "aud_2v85_en");
		gpio_direction_output(PM8058_GPIO_PM_TO_SYS(VISION_AUD_CODEC_EN), 1);
		gpio_set_value(VISION_AUD_CODEC_EN, 1);
	else
		gpio_set_value(VISION_AUD_CODEC_EN, 0);
#endif
}
...
void vision_enable_beats(int en)
{
#if 0
	pr_aud_info("%s: %d\n", __func__, en);
	if (!audio_wq) {
...
#endif
	/*
	gpio_request(PM8058_GPIO_PM_TO_SYS(VISION_AUD_SPK_SD), "AUD_SPK_EN");
	gpio_direction_output(PM8058_GPIO_PM_TO_SYS(VISION_AUD_SPK_SD), 1);
	gpio_set_value(PM8058_GPIO_PM_TO_SYS(VISION_AUD_SPK_SD), 0);
	gpio_request(PM8058_GPIO_PM_TO_SYS(VISION_AUD_CODEC_EN), "aud_2v85_en");
	gpio_direction_output(PM8058_GPIO_PM_TO_SYS(VISION_AUD_CODEC_EN), 1);
	gpio_set_value(VISION_AUD_CODEC_EN, 0);
